...
test("performRegister exibe mensagem amigável para falha de rede", async () => {
  const signUpCalls = [];
  const navigateCalls = [];
  const toastCalls = [];
  const loadingStates = [];
  ...
  assert.equal(navigateCalls.length, 0);
  assert.equal(toastCalls.length, 1);
  assert.equal(toastCalls[0].description, "Não foi possível conectar ao servidor de autenticação");
  assert.deepEqual(loadingStates, [true, false]);
});

test("performRegister normaliza mensagem de rede vinda do signUp", async () => {
  const signUpCalls = [];
  const navigateCalls = [];
  const toastCalls = [];
  const loadingStates = [];
